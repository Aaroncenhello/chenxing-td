...
function drawAmbient(now) {
  ...
  // 雷暴天气：远处闪电，纯画面效果，用固定周期而非随机避免闪烁不受控
  if (theme === "storm") {
    const lt = t % 6.4;
    if (lt < 0.08) { ctx.fillStyle = "rgba(255,255,255,.5)"; ctx.fillRect(0, 0, PW, PHt); }
    else if (lt < 0.16) { ctx.fillStyle = "rgba(255,255,255,.18)"; ctx.fillRect(0, 0, PW, PHt); }
  }
  if (GFX.amb) for (const f of AMB) {
    if (theme === "snow") { const y = (f.y + t * 18 * f.s) % PHt, x = f.x + Math.sin(t + f.p) * 6; dot(ctx, x, y, "#ffffff"); if (f.s > 0.6) dot(ctx, x + 1, y, "#e0ecff"); }
      ...
      const tw = Math.sin(t * (1.4 + f.s) + f.p * 6);
      if (tw > 0.4) { const c = f.s > 0.6 ? "#eaf7ff" : "#8fe0f0"; dot(ctx, f.x, f.y, c); if (tw > 0.85) dot(ctx, f.x + 1, f.y, "#ffffff"); }
      if (f.s > 0.75) { const y = (f.y + t * 6) % PHt, x = f.x + Math.sin(t * 0.5 + f.p) * 10; dot(ctx, x, y, "rgba(255,255,255,.4)"); }
    }
    else if (theme === "mech") {
      if (f.p < 2.5) { const cyc = 3.4 + f.s * 2, ph = (t + f.p * 3) % cyc, k = ph / cyc, y = f.y - k * 46; ctx.globalAlpha = Math.max(0, (1 - k) * 0.22); disc(ctx, f.x, y, 3 + k * 7, "#c8ccd8"); ctx.globalAlpha = 1; }
      else if ((t * 5 + f.p | 0) % 9 === 0) dot(ctx, f.x + Math.sin(t * 4 + f.p) * 5, f.y + Math.cos(t * 3 + f.p) * 5, f.s > 0.6 ? "#ffe060" : "#ffb040");
    }
    else if (theme === "blood") {
      const y = f.y - (t * 9 * f.s) % 46, x = f.x + Math.sin(t * 1.2 + f.p) * 7;
      if (f.s > 0.55) dot(ctx, x, y, "#ff6a7a");
      else { const a = Math.sin(t * 4.5 + f.p * 3); if (a > 0.35) dot(ctx, f.x + Math.sin(t * 3 + f.p) * 12, f.y + Math.cos(t * 2.3 + f.p) * 8, "#1c0a0e"); }
    }
    else if (theme === "star") {
      const tw = Math.sin(t * (1.3 + f.s) + f.p * 7);
      if (tw > 0.25) { const c = f.s > 0.65 ? "#ffffff" : f.p > 3.5 ? "#c890ff" : "#a0b8ff"; dot(ctx, f.x, f.y, c); if (tw > 0.8) dot(ctx, f.x + 1, f.y, c); }
    }
    else if (theme === "ash") {
      const y = (f.y + t * 14 * f.s) % PHt, x = f.x + Math.sin(t * 0.6 + f.p) * 8;
      rect(ctx, x, y, 1, 1, f.s > 0.6 ? "#c4c0b6" : "#8a867e"); if (f.s > 0.8) dot(ctx, x, y - 1, "#ff8a3a");
    }
    else if (theme === "ruin") {
      const y = f.y - (t * 5 * f.s) % 60, x = f.x + Math.sin(t * 0.4 + f.p) * 10, a = 0.5 + Math.sin(t * 2 + f.p * 3) * 0.3;
      if (a > 0.3) { ctx.globalAlpha = Math.min(1, a); dot(ctx, x, y, f.s > 0.6 ? "#ffe060" : "#dcdcc8"); ctx.globalAlpha = 1; }
    }
    else if (theme === "storm") {
      const y = (f.y + t * 280 * f.s) % PHt, x = ((f.x - t * 50 * f.s + y * 0.2) % PW + PW) % PW;
      rect(ctx, x, y, 1, 5 + R0(f.s * 5), f.s > 0.6 ? "rgba(200,220,255,.55)" : "rgba(160,190,230,.32)");
    }
    else if (theme === "dawn") {
      const y = f.y - (t * 8 * f.s) % 70, x = f.x + Math.sin(t * 0.5 + f.p) * 12, a = 0.4 + Math.sin(t * 1.5 + f.p * 2) * 0.3;
      ctx.globalAlpha = Math.max(0, a); dot(ctx, x, y, f.s > 0.6 ? "#ffe8a0" : "#ffd040"); ctx.globalAlpha = 1;
    }
    else if (theme === "grave") {
