...
function frame(now) {
  ...
  handleEvents();
  render(now); updateStats(); updateTeam(); updateCardbar(); updateSpells(); updateUlt(); updateInfo(); updateOffer(); updateShop(); updateEvent(); updateSynBar(); updateHud(); updatePInfo(); updateUnitCards(now); updateTut(dt); updateDlg(dt); animRoster(now);
  if (!dlg) checkOver();
