...
const CARDS = [
  ...
  { id: "critdmg", rare: 1, kind: "stat", name: "会心强化", max: 4, w: 4, desc: lv => `暴击伤害倍数 +${15 * lv}%（当前 2 → ${(2 + 0.15 * lv).toFixed(2)} 倍）` },
  { id: "res", rare: 1, kind: "stat", name: "奥术护壁", max: 4, w: 4, desc: lv => `全队法抗额外 +${10 * lv}（不叠加铁甲的法抗）` },
  { id: "loot", rare: 0, kind: "stat", name: "掠夺", max: 4, w: 4, desc: lv => `击杀获得的星尘 +${15 * lv}%` },
  ...
  { id: "sk_spear", rare: 1, kind: "skill", name: "落雷之枪", max: 5, w: 4, desc: lv => `每 ${(7 - lv * 0.5).toFixed(1)} 秒对离晨星碑最近的敌人猛击一枪，并眩晕 ${(0.8 + lv * 0.15).toFixed(2)} 秒` },
  { id: "sk_toxic", rare: 1, kind: "skill", name: "剧毒喷雾", max: 5, w: 4, desc: lv => `每 ${(8 - lv * 0.5).toFixed(1)} 秒在晨星碑周围喷出毒雾，中毒的敌人持续掉血、防御下降` },
  { id: "sk_gale", rare: 0, kind: "skill", name: "追猎箭雨", max: 5, w: 4, desc: lv => `每 ${(5.5 - lv * 0.3).toFixed(1)} 秒对 ${2 + Math.floor(lv / 2)} 个随机敌人各射一箭` },
  ...
  { id: "dodge", rare: 1, kind: "special", name: "闪避步", max: 3, w: 4, desc: lv => `每次受到攻击有 ${7 * lv}% 几率完全闪避` },
  { id: "cdr", rare: 1, kind: "special", name: "疾风节拍", max: 3, w: 3, desc: lv => `自动技能冷却额外 −${8 * lv}%` },
  { id: "wardmind", rare: 1, kind: "special", name: "心灵屏障", max: 3, w: 3, desc: lv => `受到的法术伤害 −${8 * lv}%` },
  // ---- 传说卡（每局最多见到几张，效果很强）----
  ...
  { id: "lg_bond", rare: 2, kind: "stat", name: "誓约之环", max: 1, w: 3, desc: () => "全队攻击和生命上限随伙伴数量提升：每有一名伙伴在场 +8%，没有上限" },
  { id: "lg_ward", rare: 2, kind: "stat", name: "圣域壁垒", max: 1, w: 3, desc: () => "晨星碑受到的伤害 −20%" },
  { id: "lg_focus", rare: 2, kind: "special", name: "焚天之力", max: 1, w: 3, desc: () => "对集火目标的伤害额外 +25%" },
